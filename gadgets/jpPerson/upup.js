...
(function (window) {
  ...
  Upup.prototype = {
    ...
    init: function (callback) {
      var self = this;
      var textContent = self.params.text.value;
      var textNr = textContent.replace(/\n/g,'').replace(/(.{5})(?<!$)/g,'$1\n');
      var text = textNr.split('\n');
      ...
      // 给画布填充颜色
      if (!self.params.isTranspant.checked) {
        ctx.fillStyle = self.params.bgColor.value;
      } else {
    ...
    draw: function () {
      ...
      // 监听背景透明色
      this.addEvent(this.params.isTranspant, 'input', function (e) {
        self.params.isTranspant.checked = e.target.checked
        self.refresh()
        // self.throttleDelay(self.refresh.bind(self), 500)
      })
      // 监听背景颜色
      // this.addEvent(this.params.bgColor, 'select', function (e) {
      //   // 选中的颜色序号
      //   var selectNo = document.getElementById("bgColor").selectedIndex;
      //   self.params.bgColor.value = e.target[selectNo].value;
      //   self.refresh()
      //   // self.throttleDelay(self.refresh.bind(self), 500)
      // })
      this.params.bgColor.onchange = function(e){
        var selectNo = document.getElementById("bgColor").selectedIndex;
        self.params.bgColor.value = e.target[selectNo].value;
        self.refresh();
      }
